...
buildChild.on("exit", (code) => {
	...
	const watchers = [
		...
		// JS監視
		chokidar
			.watch(`${config.assets.js}/**/*.ts`, { ignoreInitial: true })
			.on("change", (path) => {
		...
		// HTML監視
		chokidar
			.watch(`${config.assets.html}/**/*.liquid`, { ignoreInitial: true })
			.on("change", (path) => {
		...
		// 画像監視
		chokidar
			.watch(`${config.assets.images}/**/*`, { ignoreInitial: true })
			.on("change", (path) => {
		...
		// 静的ファイル監視
		chokidar
			.watch(`${config.public}/**/*`, { ignoreInitial: true })
			.on("change", (path) => {
