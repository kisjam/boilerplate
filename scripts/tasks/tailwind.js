...
/** design tokens から Tailwind のテーマ CSS を生成し SCSS として書き出す */
export default {
	name: "tailwind",
	deps: [],
	// dev: design-tokens 変更 + html/js のクラス変更でユーティリティを再生成
	watch: (ctx) => ({
		paths: [ctx.tokensPath, ctx.paths.assets.html, ctx.paths.assets.js],
		match: /\.(liquid|ts|js)$/,
	}),

	async run(ctx) {
		const startTime = performance.now();
		const { breakpoints, spacing } = ctx.tokens;
