...
test("runIncremental: affected 外のタスクは実行されない", async () => {
	const log = [];
	const tasks = [
		{ name: "a", deps: [], run: () => log.push("a:run") },
		{ name: "b", deps: ["a"], run: () => log.push("b:run") },
		{ name: "c", deps: [], run: () => log.push("c:run") },
	];
	await runIncremental(
		tasks,
		{},
		new Map([["c", { event: "change", paths: new Set(["/c.txt"]) }]]),
	);
	assert.ok(log.includes("c:run"), "c は実行される");
	assert.ok(!log.includes("a:run"), "a は実行されない");
	assert.ok(!log.includes("b:run"), "b は実行されない");
});
